Feature: request Verleihen
  As a logged in user
  I want to create an offer for a service

  Scenario: create request Verleihen successful
    Given a registerd user named "request Verleihen" with email "request@Verleihen.de" and password "test"
    And request login with the email "request@Verleihen.de" and the password "test"
    And the request Link "Neues Gesuch" is clicked
    And the request flavior "Verleihen" is selected
    And the request with offername "RequestVerleihenstest" and the category "Haustiere" and the endtime "01.01.9999" and the description "little description" is created
  ...
  Scenario: create Dienstleistung failed
    Given request login with the email "request@Verleihen.de" and the password "test"
    And the request Link "Neues Gesuch" is clicked
    And the request flavior "Verleihen" is selected
